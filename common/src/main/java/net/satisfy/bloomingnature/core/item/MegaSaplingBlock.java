...
public final class MegaSaplingBlock extends BushBlock implements BonemealableBlock {
    ...
    private void tryGrow(ServerLevel level, BlockPos pos, BlockState state, RandomSource random) {
        ...
        for (int dx = 0; dx < 4; dx++) for (int dz = 0; dz < 4; dz++) level.setBlock(origin.offset(dx, 0, dz), Blocks.AIR.defaultBlockState(), 4);
        ConfiguredFeature<?, ?> feature = level.registryAccess().registryOrThrow(Registries.CONFIGURED_FEATURE).get(this.featureKey);
        if (feature == null) {
            for (int dx = 0; dx < 4; dx++) for (int dz = 0; dz < 4; dz++) level.setBlock(origin.offset(dx, 0, dz), state, 4);
            return;
        }
        ChunkGenerator generator = level.getChunkSource().getGenerator();
        boolean placed = feature.place(level, generator, random, origin);
        if (!placed) for (int dx = 0; dx < 4; dx++) for (int dz = 0; dz < 4; dz++) level.setBlock(origin.offset(dx, 0, dz), state, 4);
    }
