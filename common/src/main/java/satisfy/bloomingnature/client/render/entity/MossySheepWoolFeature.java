...
public class MossySheepWoolFeature<T extends Sheep, F extends SheepModel<T>>
    ...
    public void render(PoseStack matrixStack, MultiBufferSource vertexConsumerProvider, int i, T sheepEntity, float f, float g, float h, float j, float k, float l) {
        ...
        if (name.equals("DEFAULT")) {
            skin = ResourceLocation.withDefaultNamespace("textures/entity/sheep_fur.png");
        } else {
            skin = BloomingNatureIdentifier.of( "textures/entity/" + name + "_sheep_fur.png");
        }
        ...
        if (sheepEntity.isInvisible()) {
            ...
            if (bl) {
                ...
                this.model.renderToBuffer(matrixStack, vertexConsumer, i, LivingEntityRenderer.getOverlayCoords(sheepEntity, 0.0f));
            }
            return;
        }
        ...
        if (sheepEntity.hasCustomName() && "jeb_".equals(sheepEntity.getName().getString())) {
            ...
            color = Sheep.getColor(DyeColor.byId(p));
        } else {
            color = Sheep.getColor(sheepEntity.getColor());
        }
        MossySheepWoolFeature.coloredCutoutModelCopyLayerRender(this.getParentModel(),this.model, skin, matrixStack, vertexConsumerProvider, i, sheepEntity, f, g, j, k, l, h, color);
    }
}